...
public class Str extends OutputStream {
	...
	public static String printDecimal(float f) {
	...
	public static boolean contains(String haystack, char needle) {
		if (haystack != null) {
	...
	public static boolean contains(char[] haystack, String needle) {
		if (haystack == null) {
			return needle == null;
		} else if (needle == null) {
			return false; // char cannot == null
		}
	...
	public static boolean containsIgnoreCase(char[] haystack, String needle) {
		if (haystack == null) {
			return needle == null;
		} else if (needle == null) {
			return false; // char cannot == null
		...
		for (int i = 0; i < haystack.length; ++i) {
			haystack[i] = Character.toUpperCase(haystack[i]);
		}
		for (char ch : needle.toCharArray()) {
	...
	public static boolean containsIgnoreCase(String[] haystack, Collection<String> needlestack) {
		if (haystack == null || needlestack == null) {
			return false;
		}
		for (String c : needlestack) {
			if (c != null) {
				if (containsIgnoreCase(haystack, c)) {
					return true;
				}
			} else {
				for (String c2 : haystack) {
					if (c2 == null) {
						return true;
					}
				}
	...
	public static boolean containsIgnoreCase(Collection<String> haystack, Collection<String> needlestack) {
		if (haystack == null || needlestack == null) {
			return false;
		}
		for (String c : needlestack) {
	...
	public static boolean startsWithIgnoreCase(String input, String check) {
		if (input == null) {
			return false;
		}
		if (input.length() >= check.length()) {
			if (input.substring(0, check.length()).equalsIgnoreCase(check)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Checks to see if <b>input</b> matches the start of any of the strings in
	 * <b>check</b>
	 *
	 * @param input
	 * @param check
	 * @return
	 */
	public static boolean startsWithPartial(String input, String... check) {
		if (input == null) {
			return false;
		}
		for (String c : check) {
			if (c.length() >= input.length()) {
				if (c.substring(0, input.length()).equals(input)) {
					return true;
				}
			}
		}
	 ...
	 * @return
	 */
	public static boolean startsWithPartial(String input, int minLength, String... check) {
	 ...
	 * @return
	 */
	public static boolean startsWithPartialIgnoreCase(String input, String... check) {
		if (input == null) {
			return false;
		}
		for (String c : check) {
			if (c.length() >= input.length()) {
				if (c.substring(0, input.length()).equalsIgnoreCase(input)) {
					return true;
				}
			}
		}
	 ...
	 * @return
	 */
	public static boolean startsWithPartialIgnoreCase(String input, int minLength, String... check) {
		if (input == null || input.length() < minLength) {
			return false;
		}
		for (String c : check) {
	...
	public static String padRight(String str, int len, String pad) {
		if (str == null) {
			throw new IllegalArgumentException("Error: String cannot be null");
		} else if(pad == null || pad.isEmpty()){
			throw new IllegalArgumentException("Error: padding cannot be empty");
		}
		StringBuilder ret = new StringBuilder(str);
		final int pl = pad.length();
		for (int i = str.length(); i < len; i += pl) {
			if(ret.length() + pl > len) {
				ret.append(pad.substring(0, len - ret.length()));
			} else {
				ret.append(pad);
			}
		}
	...
	public static String padCenter(String str, int len, String pad) {
		if (str == null) {
			throw new IllegalArgumentException("Error: String cannot be null");
		} else if(pad == null || pad.isEmpty()){
			throw new IllegalArgumentException("Error: padding cannot be empty");
		}
		final int padwid = pad.length();
		int prepad = (len - str.length()) / 2;
		
		
		StringBuilder ret = new StringBuilder();
		int w = 0;
		for(; w < prepad; w += padwid) {
			if(ret.length() + padwid > prepad) {
				ret.append(pad.substring(0, prepad - ret.length()));
				break;
			} else {
				ret.append(pad);
			}
...
//	}
	...
	public static boolean equalIgnoreCase(final String s1, final String s2, final boolean allowNull) {
		return (allowNull ? s1 == null && s2 == null : false) || (s1 != null && s2 != null && s1.equalsIgnoreCase(s2));
	}
	 ...
	 * Windows:
	 ...
	 ? (question mark)
	 * (asterisk)
	 Todo: Do not use the following reserved names for the name of a file:
	 CON, PRN, AUX, NUL, COM1, COM2, COM3, COM4, COM5, COM6, COM7, COM8, COM9, LPT1, LPT2, LPT3, LPT4, LPT5, LPT6, LPT7, LPT8, and LPT9.
	 $Mft, $MftMirr, $LogFile, $Volume, $AttrDef, $Bitmap, $Boot, $BadClus, $Secure, $Upcase, $Extend, $Quota, $ObjId, $Reparse
	 */
	private final static char fsChars[] = new char[]{'<', '>', ':', '\"', '/', '\\', '|', '?', '*', '%', '+', '[', ']'};

	/**
	 * removes any filesystem reserved characters from a string
	 */
	public static String safeFilename(String str) {
		StringBuilder result = new StringBuilder();
		for (char c : str.toCharArray()) {
			boolean fsc = c <= 31;
			for (int i = 0; i < fsChars.length && !fsc; ++i) {
				if (c == fsChars[i]) {
					fsc = true;
				}
			}
	 ...
	 *
	 * <p>This Method was written by Chas Emerick, and which avoids an
	 * OutOfMemoryError which can occur when some Java implementations are used
	 * with very large strings.<br> This implementation of the Levenshtein
	 * distance algorithm is from <a
